import {generateCarsData, MoveCar } from '../response/response'
import './cars-items.scss'
import { deleteCar, updateCar, } from '../response/response'
//import { deleteCar, updateCar, carRunAnimation, returnCar } from '../response/response'
...
export default function carsItems(carName: string, carColor: string, id: number) {
  ...
  if (carsItemsCount.length <= 6) {
    ...
    //startButton?.addEventListener('click', () => {window.requestAnimationFrame(moveCar.carRunAnimation())})
    startButton?.addEventListener('click', () => {moveCar.startEngine()})
    stopButton?.addEventListener('click', () => {moveCar.returnCar()})
    //startButton?.addEventListener('click', () => {carRunAnimation(event)})
    //stopButton?.addEventListener('click', () => {returnCar(event)})
  }
}
